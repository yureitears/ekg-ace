...
const FAKE = [
  { name: "Dr. Patel", score: 2840, streak: 42, acc: 94 },
  { name: "MedStudent_22", score: 2610, streak: 31, acc: 91 },
  { name: "ECGninja", score: 2455, streak: 28, acc: 89 },
  { name: "QRSqueen", score: 2310, streak: 24, acc: 87 },
  { name: "ST_elevation", score: 2180, streak: 19, acc: 85 },
  { name: "Sgarbossa_fan", score: 2050, streak: 17, acc: 83 },
...
const Leaderboard = () => {
  ...
  const me = { name: "You", score: myScore, streak: stats.streak, acc: computeAccuracy(stats), self: true };
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="container max-w-3xl py-10">
        <div className="mb-8">
          <p className="font-mono-clinical text-xs uppercase tracking-widest text-primary">Global rankings</p>
          <h1 className="mt-1 font-display text-3xl font-bold">Leaderboard</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Demo board with simulated peers. Connect Lovable Cloud to enable real users, friends and live ranks.
          </p>
        </div>
        ...
        <div className="card-elevated overflow-hidden rounded-xl">
          ...
          {board.map((row, i) => (
            ...
            >
              ...
              <span className={`font-medium ${(row as any).self ? "text-primary" : ""}`}>{row.name}{(row as any).self && " · you"}</span>
